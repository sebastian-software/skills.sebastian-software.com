# Inbound Profile

Treat the profile as a buyer-facing trust path, not a duplicated CV or a crowded
link directory.

## Keep profile ownership explicit

Use the optional the profile routes for field-level profile doctrine:
professional evidence, photo, banner, headline, About, Experience, skills,
recommendations, localization, and cross-channel consistency. Without that
skill, audit those fields against the target buyer, promise, evidence, and
acquisition path defined here. This route owns the acquisition overlay: the
target-buyer path, campaign proof, Featured choices, primary action, and
downstream measurement.

For end-to-end social-selling work:

1. Establish the target buyer, problem, promise, proof, and useful next step.
2. Pass that context and the consultant's evidence to the profile routes for
   any profile audit or rewrite.
3. Return here to prioritize proof and action for the active acquisition path.

Do not maintain a second field-by-field profile formula in this reference.

## Define the visitor path

Design each visible section to answer one question:

1. **Identity** - Is this clearly the right person and a credible human?
2. **Relevance** - Who do they help, in what situation, toward what outcome?
3. **Understanding** - Do they describe the buyer's problem naturally?
4. **Proof** - What work, results, recommendations, or artifacts support the
   promise?
5. **Method** - How do they approach the work and what is the useful first step?
6. **Action** - What should a qualified visitor do next?

Choose one primary action. Use secondary links only when they support a distinct
visitor intent.

## Apply the acquisition overlay

Review the current platform surface before prescribing campaign changes. Then
assess:

- **Positioning handoff:** the profile content from the profile routes matches
  the target buyer, promise, vocabulary, and proof established here.
- **Featured path:** one or two high-value proof or conversion items support the
  primary visitor path; add another only for a deliberate campaign.
- **Primary action:** a qualified visitor can identify the useful next step
  without choosing among competing calls to action.
- **Campaign continuity:** profile, posts, messages, lead magnet, landing page,
  and meeting use compatible promises, proof, and language.
- **Operational path:** links work on mobile, preserve useful attribution, and
  do not remove durable proof merely to make a campaign item dominant.

## Align profile and campaign

Make the promise, vocabulary, proof, and next step consistent across profile,
posts, messages, lead magnet, landing page, and call. When a campaign changes,
update the profile intentionally and record the previous variant.

Do not remove useful proof merely to make one campaign link visually dominant.
Rank items by buyer value and test the resulting path.

## Review with evidence

Use a before-and-after snapshot and track downstream movement:

- qualified profile visits
- relevant connection requests or follows
- Featured or contact-path clicks
- replies and conversations attributed to the profile
- qualified meetings and opportunities

Do not declare a profile successful from views alone. Segment by target-buyer
quality when the data allows it.

## Quality Check

- Can the intended buyer identify themselves and the likely value quickly?
- Does the profile sound like the person behind the account?
- Is each claim supported or clearly framed as an approach?
- Did the profile routes own any field-level audit or rewrite?
- Is there a single obvious next step for a qualified visitor?
- Are volatile LinkedIn features and dimensions verified from current official
  guidance?
- Does the mobile view preserve meaning, proof, and action?
