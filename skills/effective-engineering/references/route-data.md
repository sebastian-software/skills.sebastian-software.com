...
3. Model the authoritative facts, identities, relationships, lifecycle, and
   ownership before selecting storage. Read
   [Storage and models](storage-and-models.md) for model and datastore choices
   driven by access patterns and invariants.
4. Define correctness per user or system flow. Read
   [Transactions and consistency](transactions-and-consistency.md) when work
   involves atomicity, isolation, concurrent updates, idempotency,
   read-your-writes, stale reads, or coordination across boundaries.
5. Read
   [Replication, partitioning, and streams](replication-partitioning-and-streams.md)
...
6. Read [Evolution and operations](evolution-and-operations.md) for schema
   changes, backfills, dual-running, cutovers, reconciliation, recovery,
...
- Overall system boundaries, service ownership, quality attributes, deployment
  topology, and cross-system migration direction are the Architecture route.
  This route owns data model and data-behavior decisions inside or between those
  agreed boundaries.
- Focused database, transaction, migration, async, retry, and failure-path test
  design is the Testing route; this route defines the data guarantee and failure
  scenario the test must preserve.
- Repository-wide diagnosis, prioritization, executable change planning, and
  behavior-preserving migration across languages, runtimes, frameworks,
  platforms, or major APIs belong to `effective-delivery`; data compatibility
  and cutover guarantees stay here.
- A durable datastore, consistency, ownership, or migration choice that needs an
  ADR belongs to `effective-product`.
...
system-boundary findings to the Architecture route and keep this route focused
