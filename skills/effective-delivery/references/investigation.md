...
- an authorized repository fix, refactor, or focused plan ->
  the audit route, or the orchestration route when the work must be coordinated;
- focused non-frontend regression evidence -> `effective-engineering`;
- frontend-only diagnosis or implementation -> `effective-web`;
- PR-scoped review or maintenance -> the review route;
- dependency update delivery -> the dependencies route;
- documentation correction -> the docs route;
- product intent -> `effective-product`;
- system direction -> `effective-engineering`, with `effective-product` after a
