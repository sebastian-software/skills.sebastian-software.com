...
- **Selects/comboboxes:** evaluate the customizable native `select` (`appearance: base-select`, `::picker(select)`, rich option content) before building a custom combobox, keeping the existing form semantics and JS interface. Where base-select support is not yet reliable, plan a fallback to the standard native control, and test mobile OS picker behavior, fallback parsing, width behavior, option content, and form submission.
