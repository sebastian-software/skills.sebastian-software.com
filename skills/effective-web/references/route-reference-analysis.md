...
Turn supplied references into an inspectable specification. Preserve what was
actually observed, label inference, and extract portable behavior rather than
copying a source's brand, content, or complete composition.

Applies to live websites, local HTML, screenshots, screen recordings, videos,
prototypes, and reference packs.
...
3. Read [Evidence capture](evidence-capture.md). Capture only the states needed
   to support the requested outcome, but cover the whole relevant experience
   rather than mistaking one hero frame for the system.
...
7. Read [Specification and handoff](specification-and-handoff.md). Produce the
   smallest output that the next owner can implement or decide against. Attach
   evidence to requirements and label every unresolved state.
...
- Browser-facing implementation, accessibility, responsive behavior, motion
  construction, performance, and frontend verification are the matching
  implementation routes here, once the specification is accepted.
- Comparison of a produced work against its sources for copying risk, asset
  provenance, and release judgment is the Originality route.
- User and product research, problem framing, experience modeling, information
  architecture, and direction selection belong to `effective-product`.
  Reference analysis supplies evidence; it does not decide the product problem.
  A durable reference-derived direction is recorded there as an ADR.
- Market positioning, campaign claims, audience, and proof decisions belong to
  `effective-marketing`; this route may analyze their visible execution only.
- Keep general internet research, competitor strategy, legal clearance, and
  unauthorized reproduction outside this route.
