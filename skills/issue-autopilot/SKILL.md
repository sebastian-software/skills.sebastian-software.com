...
changing repository state. Activate this skill only when at least one condition is
true:
...
Treat the request as targeted, and therefore outside this skill, when it identifies
...
1. Stop applying this skill's queue workflow. Do not inspect the broader backlog,
   rank or claim other work, reconcile unrelated PRs, create a monitor, or introduce
   this skill's worktree and ready-state conventions.
2. Route a named software change through `effective-workflow` and standalone PR
   maintenance through `pr-review`, plus only the domain skills the target requires.
...
Read [references/progress-updates.md](references/progress-updates.md) before the
...
Load and apply `metro-english` before posting team-facing GitHub or Linear text.
Use it for:
...
Include `metro-english` in every implementation or repair agent's assignment so
the agent applies it before posting, rather than leaving the language pass to the
orchestrator after publication.
...
2. Remove `ready-to-merge` immediately when the invariant in
   [references/ready-state.md](references/ready-state.md) is false.
...
[references/ranking.md](references/ranking.md), then run:
...
Read [references/worktree-safety.md](references/worktree-safety.md) before creating,
...
- an explicit instruction to load `metro-english` for PR descriptions, issue
...
5. Update required docs and work logs. Link the PR to the issue using native closing
   ...
   `metro-english` to the PR description and tracker/PR communication without
...
Read [references/ready-state.md](references/ready-state.md) before adding or
...
Read [references/monitoring.md](references/monitoring.md) before creating or waking
...
Use `effective-workflow` for a newly named software change and `pr-review` for a
standalone named PR. Use `metro-english` for team-facing collaboration text after
the technical substance is settled. This skill owns only delegated queue selection,
queue-owned implementation coordination, and ongoing queue reconciliation; it does
not merge PRs or absorb targeted work.
