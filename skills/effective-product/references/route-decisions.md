...
4. Read [ADR format and lifecycle](adr-format.md), then write the smallest
   complete record. For language, brand, content, or marketing decisions, also
   read [Communication decisions](communication-decisions.md).
...
When the repository has no decision convention and the work genuinely needs one,
use `docs/adr/` with zero-padded Markdown filenames such as
...
- Repository-wide decision discovery, audit, and prioritization, and unresolved
  system direction, belong to `effective-delivery` and `effective-engineering`
  respectively; use this route once a durable decision needs a record.
- Implementation work belongs to the discipline that owns that surface.
