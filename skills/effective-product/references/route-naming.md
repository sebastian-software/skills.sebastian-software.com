# Route: Product and Feature Naming
...
2. If the product thesis or target user is unresolved, resolve it on the
   Strategy route first; naming cannot repair unclear positioning.
3. Choose two to four relevant name territories. Load
   [Name generation](name-generation.md) for territories, sound, form, and
   anti-patterns.
...
6. Research only the surviving candidates. Load
   [Screening and verification](screening-and-verification.md) for current
...
- Audience, product thesis, differentiation, and portfolio architecture are
  resolved on the Strategy route before naming.
- Post-selection messaging, launch work, and naming that belongs to an
  individual consultant's positioning rather than a product identity belong to
  `effective-marketing`.
- Website disclosures and jurisdiction-specific launch requirements, and
  comparison of a produced identity against supplied visual and verbal
  references for source overlap, belong to `effective-web`. Neither replaces
  trademark counsel; keep trademark similarity with this route's preliminary
  screening and qualified counsel.
