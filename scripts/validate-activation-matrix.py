...
checker keeps that post-consolidation routing contract well formed.
...
def published_disciplines() -> set[str]:
    return {
        path.parent.name
...
def validate_cases(
    ...
    for index, case in enumerate(cases):
        ...
        if not isinstance(case, dict) or set(case) != CASE_FIELDS:
            errors.append(
                f"{location} keys must be exactly ['name', 'owner', 'prompt']"
            )
            continue
...
def main() -> int:
    ...
    if isinstance(matrix, dict):
        if matrix.get("version") != 2:
            errors.append(f"{MATRIX_PATH}: version must be 2")
        ...
        validate_cases(matrix.get("cases"), published_disciplines(), errors)
    ...
    print(
        ...
        f"{len(owners - {NO_OWNER})} disciplines; "
